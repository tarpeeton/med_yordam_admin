...
export const useAddressStore = create<AddressState>((set, get) => ({
  ...
  deleteAddress: (id: string) => {
    ...
    const doctorId = useClinicProfileStore.getState().id;
  ...
  save: async (): Promise<boolean> => {
    try {
      ...
      const payload = {
        id: DoctorID,
        receptionTime: mapData.map((entry) => ({
      ...
      if (response.data?.data?.receptionTimes) {
        get().setAllData(response.data.data.receptionTimes);
      }
